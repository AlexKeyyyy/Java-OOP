...
public class Translator1
{
    ...
    public Translator1(final Dictionary dictionary, final File textPath) throws FileReadException {
    ...
    public List<String> getTranslatedText() throws FileReadException {
        if (text.isEmpty()) {
            throw new FileReadException(INCORRECT_DATA_EXCEPTION);
        }
        ...
        for (int i = 0; i < textSize; i++) {
            ...
            result.add(dictionary.wordMap.getOrDefault(previousEnglishWordFromDict, text.get(i)));
            i = newIndex;
        }
        return result;
    }
    ...
    private String getEnglishWordFromDictionary(final String word) {
        String currentWord = "";
        for (var el : dictionary.wordMap.keySet()) {
            if (el.equals(word)) {
                return el;
            }
